...
describe('redisClient', () => {
  ...
  //     sinon.stub(redisClient, 'pingAsync').resolves('PONG');
  //     const result = await redisClient.isAlive();
  //     expect(result).to.be.true;
  //     redisClient.pingAsync.restore();
  //   });

  //   it('should return false if Redis does not respond to PING', async () => {
  //     sinon.stub(redisClient, 'pingAsync').rejects(new Error('Redis not reachable'));
  //     const result = await redisClient.isAlive();
